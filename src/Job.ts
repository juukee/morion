...
export class Job<DATA = unknown | void> {
	...
	cancel(error?: Error | string) {
		this.morion.emit(`cancel:${this.attrs.name}`, this);
	...
	private async fetchStatus(): Promise<void> {
		const dbJob = await this.morion.db.getJobs({ _id: this.attrs._id });
	...
	async save(): Promise<Job> {
		...
		await this.morion.ready;
		return this.morion.db.saveJob(this as Job);
	}
	...
	remove(): Promise<number> {
		return this.morion.cancel({ _id: this.attrs._id });
	}
	...
	async isExpired(): Promise<boolean> {
		...
		const definition = this.morion.definitions[this.attrs.name];
	...
	async touch(progress?: number): Promise<void> {
		...
		await this.morion.db.saveJobState(this);
	}
	...
	async run(): Promise<void> {
		...
		try {
				...
				await new Promise<void>((resolve, reject) => {
					this.forkedChild = fork(
						...
						[
							...
							this.morion.definitions[this.attrs.name].filePath || ''
						],
						forkHelper.options
					);
			...
			this.morion.emit('success', this);
			this.morion.emit(`success:${this.attrs.name}`, this);
			...
			this.morion.emit('fail', error, this);
			this.morion.emit(`fail:${this.attrs.name}`, error, this);
			...
			this.morion.emit('complete', this);
			this.morion.emit(`complete:${this.attrs.name}`, this);
			log(
