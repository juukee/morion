...
import type { IMorionConfig } from './types/MorionConfig';
import type { IDatabaseOptions, IDbConfig, IMongoOptions } from './types/DbOptions';
import type { IMorionStatus } from './types/MorionStatus';
 ...
 * @class
 */
export class Morion extends EventEmitter {
	readonly attrs: IMorionConfig & IDbConfig;
	...
	async getRunningStats(fullDetails = false): Promise<IMorionStatus> {
		if (!this.jobProcessor) {
			throw new Error('morion not running!');
		}
		return this.jobProcessor.getStatus(fullDetails);
	}

	/**
	 * @param {Object} config - Morion Config
	 * @param {Function} cb - Callback after Morion has started and connected to mongo
	 */
	constructor(
		config: {
			name?: string;
	...
	/**
	 * Use existing mongo connectino to pass into morion
	 * @param mongo
	 * @param collection
	 */
	async mongo(mongo: Db, collection?: string): Promise<Morion> {
	 ...
	 * @param query
	 */
	sort(query: { [key: string]: SortDirection }): Morion {
		log('Morion.sort([Object])');
	...
	async cancel(query: Filter<IJobParameters>): Promise<number> {
		log('attempting to cancel all Morion jobs', query);
		try {
	 ...
	 * @param name
	 */
	name(name: string): Morion {
		log('Morion.name(%s)', name);
	 ...
	 * @param num
	 */
	maxConcurrency(num: number): Morion {
		log('Morion.maxConcurrency(%d)', num);
	 ...
	 */
	defaultConcurrency(num: number): Morion {
		log('Morion.defaultConcurrency(%d)', num);
	 ...
	 * @param num
	 */
	lockLimit(num: number): Morion {
		log('Morion.lockLimit(%d)', num);
	 ...
	 * @param num
	 */
	defaultLockLimit(num: number): Morion {
		log('Morion.defaultLockLimit(%d)', num);
	 ...
	 * @param ms
	 */
	defaultLockLifetime(ms: number): Morion {
		log('Morion.defaultLockLifetime(%d)', ms);
	...
	async purge(): Promise<number> {
		...
		log('Morion.purge(%o)', definedNames);
	...
	define(
		...
		if (this.definitions[name]) {
			log('overwriting already defined morion job', name);
		}
	...
	create(name: string, data?: unknown): Job<any> {
		log('Morion.create(%s, [Object])', name);
	...
	async every(
		 ...
		 */
		log('Morion.every(%s, %O, %O)', interval, names, options);
		...
		log('Morion.every(%s, %s, %O)', interval, names, options);
	...
	async schedule(
		...
		if (typeof names === 'string') {
			log('Morion.schedule(%s, %O, [%O])', when, names);
	...
	async start(): Promise<void> {
		log(
			'Morion.start called, waiting for morion to be initialized (db connection)',
			this.attrs.processEvery
		);
		await this.ready;
		if (this.jobProcessor) {
			log('Morion.start was already called, ignoring');
			return;
		}
	...
	async stop(): Promise<void> {
		if (!this.jobProcessor) {
			log('Morion.stop called, but morion has never started!');
			return;
		}

		log('Morion.stop called, clearing interval for processJobs()');

		const lockedJobs = this.jobProcessor.stop();

		log('Morion._unlockJobs()');

		lockedJobs?.forEach(job => job.cancel(new Error('morion stopped')));
...
export * from './types/MorionConfig';
