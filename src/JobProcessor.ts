...
import type { IMorionJobStatus, IMorionStatus } from './types/MorionStatus';
import type { IJobDefinition } from './types/JobDefinition';
import type { Morion, JobWithId } from './index';
import type { IJobParameters } from './types/JobParameters';
import { Job } from './Job';
import { JobProcessingQueue } from './JobProcessingQueue';

const log = debug('morion:jobProcessor');
...
export class JobProcessor {
	...
	constructor(
		...
		this.jobQueue = new JobProcessingQueue(morion);
		this.processInterval = setInterval(() => this.process(), processEvery);
		this.process();
	}

	async getStatus(fullDetails = false): Promise<IMorionStatus> {
		const jobStatus = Object.keys(this.morion.definitions).reduce((obj, key) => {
			obj[key] = {
				...this.jobStatus[key],
				config: this.morion.definitions[key]
			};
			return obj;
		}, {}) as IMorionJobStatus;
		...
		return {
			...
			queueName: this.morion.attrs.name,
			totalQueueSizeDB: await this.morion.db.getQueueSize(),
			internal: {
	...
	async process(extraJob?: JobWithId): Promise<void> {
		...
		// Prevents race condition with 'Morion.stop' and already scheduled run
		if (!this.isRunning) {
		...
		if (!extraJob) {
			...
			// Go through each jobName set in 'Morion.process' and fill the queue with the next jobs
			await Promise.all(
				Object.keys(this.morion.definitions).map(async jobName => {
			...
			this.morion.definitions[extraJob.attrs.name] &&
	...
	shouldLock(name: string): boolean {
		const jobDefinition = this.morion.definitions[name];
		let shouldLock = true;
		// global lock limit
	...
	async lockOnTheFly(): Promise<void> {
		...
		try {
			...
			if (job) {
				...
				const resp = await this.morion.db.lockJob(job);
				...
				if (resp) {
					...
					const jobToEnqueue = new Job(this.morion, resp, true) as JobWithId;
					...
					if (!this.shouldLock(jobToEnqueue.attrs.name)) {
						...
						this.morion.db.unlockJob(jobToEnqueue);
	...
	private async findAndLockNextJob(
		...
		const result = await this.morion.db.getNextJobToRun(jobName, this.nextScanAt, lockDeadline);

		if (result) {
			log.extend('findAndLockNextJob')(
				'found a job available to lock, creating a new job on Morion with id [%s]',
				result._id
			);
			return new Job(this.morion, result, true) as JobWithId;
		}
	...
	private async jobQueueFilling(name: string): Promise<void> {
		...
		try {
			...
			const job = await this.findAndLockNextJob(name, this.morion.definitions[name]);
	...
	private async runOrRetry(job: JobWithId): Promise<void> {
		...
		const jobDefinition = this.morion.definitions[job.attrs.name];
		...
		if (
			...
			try {
				...
				const checkIfJobIsStillAlive = () =>
					// check every "this.morion.definitions[job.attrs.name].lockLifetime / 2"" (or at mininum every processEvery)
					new Promise<void>((resolve, reject) => {
								...
								reject(
									new Error(
										`execution of '${job.attrs.name}' canceled, execution took more than ${
											this.morion.definitions[job.attrs.name].lockLifetime
						...
						}, Math.max(this.processEvery / 2, this.morion.definitions[job.attrs.name].lockLifetime / 2));
					});
